    ...
    <div class="page-header">
        ...
        <div class="page-btn">
            <a href="{{route('cms.create')}}" class="btn btn-primary" >
                <i class="ti ti-circle-plus me-1"></i>Add cms
            </a>
        </div>
    </div>
    <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between flex-wrap row-gap-3 bg-primary">
            <div>
                <h4 class="text-light">cms List</h4>
            </div>           
        </div>
        <div class="card-body p-3">
            <div class="table-responsive">
                <table class="table " id="cms_table">
...
@section('script')
  <script src="{{ url('assets/plugins/sweetalert/sweetalert2.all.min.js')}}" ></script>
    ...
    <script>   
        ...
        var cmsTable = $('#cms_table').DataTable({
            orderCellsTop: true,
            fixedHeader: true,
            info : false,
            ajax : "{{route('cms.get_cms')}}",
            order : [[ 0, "desc" ]],
            fixedHeader: true,
            processing : true,
            responsive : true,
        ...
        $(document).on('click', '.status-toggle-btn', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        $('#cms_table').DataTable().ajax.reload(null, false); 
                    } else {                     
