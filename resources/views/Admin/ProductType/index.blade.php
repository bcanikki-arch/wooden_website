...
@section('content')
    ...
    <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between flex-wrap row-gap-3 bg-primary">
            <div>
                <h4 class="text-light">Product Type List</h4>
            </div>           
        </div>
        <div class="card-body p-3">
            <div class="table-responsive">
                <table class="table " id="producttype_table">
...
@section('script')
    ...
    <script>   
        ...
        var brandTable = $('#producttype_table').DataTable({
            orderCellsTop: true,
            fixedHeader: true,
            info : false,
            ajax : "{{route('producttype.get_producttype')}}",
            order : [[ 0, "desc" ]],
            fixedHeader: true,
            processing : true,
            responsive : true,
        ...
        $(document).on('click', '.status-toggle-btn', function() {
            ...
            $.ajax({
                url: 'product-type/update-status/' + brandId, 
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    status: nextStatus 
                ...
                success: function(response) {
                    if (response.success) {
                        $('#producttype_table').DataTable().ajax.reload(null, false); 
                    } else {                     
