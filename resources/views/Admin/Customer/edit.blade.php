...
<div class="page-header">
    ...
    <div class="page-btn mt-0">
        <a href="{{ route('customer') }}" class="btn btn-secondary"><i data-feather="arrow-left" class="me-2"></i>Back to
            Customer</a>
    </div>
</div>
<form action="{{ route('customer.update') }}" method="POST" class="add-product-form" enctype="multipart/form-data">
    @csrf
                ...
                <h2 class="accordion-header">
                    <div class="accordion-button collapsed bg-primary">
                        <div class="d-flex align-items-center justify-content-between flex-fill">
                            <h5 class="d-flex align-items-center text-light"><span>Edit Customer</span></h5>
                        </div>
                    </div>
                </h2>
                    ...
                    <div class="accordion-body border-top">
                        ...
                        <div class="row">
                            <div class="col-sm-6 col-12">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" name="name"
                                        value="{{ old('name', $customer->name) }}" id="name">
                                </div>
                            </div>
                            <div class="col-md-12 mt-3">
                                <label class="form-label">Mobile Number</label>
                                {{-- Added name="mobile" --}}
                                <input type="text" name="mobile" value="{{ old('mobile', $customer->mobile) }}" class="form-control @error('mobile') is-invalid @enderror" placeholder="Enter mobile number">
                                @error('mobile')
                            ...
                            <div class="col-md-12 mt-3">
                                ...
                                {{-- Added name="email" --}}
                                <input type="email" name="email" value="{{ old('email', $customer->email) }}" class="form-control @error('email') is-invalid @enderror" placeholder="Enter email">
                                @error('email')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                            ...
                            <div class="col-md-12 mt-3">
                                ...
                                <textarea name="address" class="form-control @error('address') is-invalid @enderror" value="{{ old('address', $customer->address) }}">{{ $customer->address }}</textarea>
                                @error('address')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="select" name="state_id" id="state_id">
                                            ...
                                            <option value="{{ $state->id }}"
                                                @if ($state->id ==  $customer->state_id ?? null)
                                                selected
                                                @endif
                                                >
                                                {{ $state->name }}
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="select" name="city_id" id="city_id">
                                            ...
                                            <option value="{{ $city->id }}"
                                                {{-- This line ensures the correct city is pre-selected --}}
                                                @if ($city->id == $customer->city_id ?? null)
                                                selected
                                                @endif
                                                >
                                                {{ $city->city }}
                            ...
                            <div class="col-md-12 mt-3">
                                ...
                                <textarea name="details" value="{{ old('details', $customer->details) }}" class="form-control @error('details') is-invalid @enderror">{{ $customer->details }}</textarea>
                                @error('details')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
